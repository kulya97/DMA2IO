...
//
//		}
		void GenerteChannelDataCycle(u32 *addr, u8 Channel, u32 highCnt,
				u32 lowCnt, u32 cycleCnt) {
...
//发送数据
		...
		static int SetupIntrSystem(INTC * IntcInstancePtr, XAxiDma * AxiDmaPtr,
			...
			XScuGic_SetPriorityTriggerType(IntcInstancePtr, TxIntrId, 0xA0,
					0x3);

			XScuGic_SetPriorityTriggerType(IntcInstancePtr, RxIntrId, 0xA0,
					0x3);
			/*
				...
				for (Pkts = 0; Pkts < NUMBER_OF_BDS_PER_PKT; Pkts++) {
					...
					if (Status != XST_SUCCESS) {
						xil_printf(
								...
								(unsigned int) BufferAddr, (UINTPTR) BdCurPtr,
								Status);
						return XST_FAILURE;
					}
					//设置bd长度字段
...
//								TxRingPtr->MaxTransferLen);
					...
					//依次添加bd到bdring
					BdCurPtr = (XAxiDma_Bd *) XAxiDma_BdRingNext(TxRingPtr,
							BdCurPtr);
				}
			}
